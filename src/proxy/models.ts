...
export interface ClaudeAuthStatus {
  ...
  email?: string
  // Extended account info surfaced in /health (read from .claude.json oauthAccount).
...
export async function getClaudeAuthStatusAsync(profileId?: string, envOverrides?: Record<string, string>): Promise<ClaudeAuthStatus | null> {
  ...
  c_promise = (async () => {
    try {
      ...
      if (parsed.loggedIn) {
        // `claude auth status` returns only loggedIn/email/subscriptionType(null),
        // so surface the full account info from .claude.json oauthAccount for /health.
        ...
        try {
          ...
          if (acct) {
            if (!parsed.email && acct.emailAddress) parsed.email = acct.emailAddress
            if (!parsed.subscriptionType && (acct.organizationRateLimitTier || acct.organizationType)) {
              parsed.subscriptionType = acct.organizationRateLimitTier || acct.organizationType
            }
            if (acct.organizationRateLimitTier) parsed.rateLimitTier = acct.organizationRateLimitTier
            if (acct.subscriptionCreatedAt) parsed.subscriptionCreatedAt = acct.subscriptionCreatedAt
            if (acct.accountCreatedAt) parsed.accountCreatedAt = acct.accountCreatedAt
            if (acct.accountUuid) parsed.accountUuid = acct.accountUuid
            if (acct.billingType) parsed.billingType = acct.billingType
          }
        } catch {}
      }
      if (cache) {
